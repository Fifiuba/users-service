...
from users_service.utils import (
...
async def get_opinions_user(
    rq: Request, id: int, user_type: str, db: Session = Depends(database.get_db)
):
    try:
...
async def login_user(
    ...
    try:
        ...
        events.create_event(
            "Login with Email",
            "A user login in the system via Email",
            "info",
            [
                "type:INFO",
                "type:INFO",
    ...
    try:
        ...
        if isNewUser:
            events.create_event(
                ...
                "A user registers the systems with google",
                "info",
                [
                    "type:INFO",
        ...
        events.create_event(
            "Login with google",
            "A user login in the system via google",
            "info",
            [
                "type:INFO",
                "endpoint:/users/loginGoogle",
...
async def block_user(
    ...
    try:
        ...
        if userBlock.block:
            ...
            events.create_event(
                "Unlock User",
                "A user was unblock by an admin",
                "info",
                [
                    "type:INFO",
                    "endpoint:/users/block",
...
@user_router.post("/restorePassword/{email}", status_code=status.HTTP_200_OK)
